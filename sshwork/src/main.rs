...
use std::process::{Child, Command}; // Child process runner.
use std::io::{self, BufRead};
use std::io::Write;



const SSH_DIR: &str = "%HOMEPROFILE%/.ssh";
...
fn get_ssh_info() -> Result<IsInstalled, Error> {
    ...
    let bins = vec! ["ssh","ssh-keygen","ssh-agent","ssh-add" ];
    for bin in bins.iter() {
    //let bin = "ssh";
    let child_output= Command::new("where")
    .arg(bin)
    .output()
    .map_err(|err| {
        format!("failed to run process: `where {bin}`, error: `{err}`")
    })?;
    if child_output.status.success() {
        let child_stdout = String::from_utf8(child_output.stdout)
        .map_err(|err| format!("failed to parse process stdout: `where {bin}`, error: `{err}`"))?;
    ...
    if ssh_checks == 4{
        ...
        if ssh_version.status.success(){
            
            //println!("status: {}", String::from_utf8_lossy(&ssh_version.stdout));
            println!("status: {}", ssh_version.status);
            

            let sshv_output = String::from_utf8(ssh_version.stderr)
            .map_err(|error| format!("failed to parse process stdout: `ssh -V`, error: `{error}`"))?;
            let sshv_out = sshv_output.trim();
            println!("SSH version {sshv_out}");
            
            let ssh_path = Path::new("%HOMEPROFILE%/.ssh");
            let sp:std::borrow::Cow<'_, str> = ssh_path.to_string_lossy();
            println!("{sp}");
            
            if ssh_path.try_exists().is_ok(){
                println!("[✓] SSH directory: found");
                let ssh_config = Path::new("%HOMEPROFILE%/.ssh/config");
...
fn list_keypairs() -> Result<(), Error> {
    let ssh_read: Vec<DirEntry> = read_ssh_dir()?;
...
//fn get_ssh_dir() -> Result<String,Error>{
    //let mut ssh_dir_string = "";

 //   let ssh_dir_output= Command::new("dir")
 //   .args([".ssh","/AD","/s"])
 //   .output()
//    .map_err(|err| {
//        format!("failed to run process: `dir`, error: `{err}`")
 //   })?;

 //   let sshd_output = String::from_utf8(ssh_dir_output.stderr)
  //          .map_err(|error| format!("failed to run process: `dir`, error: `{error}`"))?;


    //return ssh_dir_string.to_string();
 //   Ok(sshd_output)
//}

fn main() -> Result<(), Error> {
    //let user_ssh_dir:String = get_ssh_dir()?;
// get .ssh dir and pass to functions <<<<<<<<<<<<<<<<<<<
